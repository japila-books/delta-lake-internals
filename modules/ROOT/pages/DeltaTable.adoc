...
`DeltaTable` is the main class to <<operators, programmatically access Delta tables>>.

You can create `DeltaTable` instances for existing Delta tables using <<forPath, DeltaTable.forPath>> utility (static method).
