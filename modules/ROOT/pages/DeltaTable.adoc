...
forName xref:DeltaTableUtils.adoc#isDeltaTable[checks whether the given table name is of a Delta table] and, if so, creates a DeltaTable with the following:

* Dataset that represents loading data from the specified table name (using `SparkSession.table` operator)

* xref:DeltaLog.adoc#forTable[DeltaLog] of the specified table
...
forName is used internally when DeltaConvert utility is used to xref:DeltaConvert.adoc#executeConvert[executeConvert].
...
forPath is used internally in <<convertToDelta, DeltaTable.convertToDelta>> (via xref:DeltaConvert.adoc[] utility).
