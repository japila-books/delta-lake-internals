...
`DeltaSource` is the streaming source of <<DeltaDataSource.adoc#, delta data source>> that can be used for streaming queries in Spark Structured Streaming.
...
Used when `DeltaSource` is requested for the <<getOffset, getOffset>>.
