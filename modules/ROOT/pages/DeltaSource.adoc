...
NOTE: `isStartingVersion` flag simply adds <<getSnapshotAt, gets the state (snapshot)>> before <<getChanges-filterAndIndexDeltaLogs, (filtered out) indexed AddFiles>> (when enabled) or not.
