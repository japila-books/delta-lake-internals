...
`OptimisticTransactionImpl` manages an `newMetadata` internal registry for a new <<Metadata.adoc#, Metadata>> that should be committed with this transaction.

`newMetadata` is initially undefined (`None`) and can be <<updateMetadata, updated>> once (and before <<TransactionalWrite.adoc#hasWritten, writing out any files>>).
