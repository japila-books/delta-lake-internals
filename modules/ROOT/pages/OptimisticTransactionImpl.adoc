...
`commit` registers the <<GenerateSymlinkManifest.adoc#, GenerateSymlinkManifest>> post-commit hook when...FIXME

`commit`...FIXME

[[commit-runPostCommitHooks]]
In the end, `commit` <<runPostCommitHooks, runs post-commit hooks>>.
